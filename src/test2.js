...
router.get('/loctheogia', async (req, res, next) => {
    try {
        ...
        const filteredProducts = products.filter(product => {
            return product.price >= min_price && product.price <= max_price && product.quantity > 0;
        });

        // Sắp xếp sản phẩm theo số lượng tăng dần
        const sortedProducts = filteredProducts.sort((a, b) => a.quantity - b.quantity);

        return res.status(200).json({ getProductsByPriceMinMax: sortedProducts.length, result: sortedProducts });
    } catch (error) {
...
router.get('/search', async (req, res, next) => {
    try {
        ...
        const result = products.filter(product => product.name.toLowerCase().includes(key.toLowerCase()));
